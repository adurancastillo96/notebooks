...
2. Review the notebook template: `notebook_template.ipynb`
...
├── notebook_template.ipynb   ← Master notebook template
...
├── src/                      ← Authored notebooks (category-name.ipynb)
...
| 5. Review | Validate, lint, fix, polish | `@notebook-reviewer` |
...
Every PR that modifies notebooks in `src/` is automatically checked:

| Check | What It Validates |
|-------|------------------|
| **Structure Lint** | Template sections present and in correct order |
...
| **Auto-Label** | PRs auto-labeled by content type |
...
All notebooks in `src/` follow the pattern: `category-name.ipynb`

Examples:
- `vertex-ai-custom-training.ipynb`
- `bigquery-ml-forecasting.ipynb`
- `aerospace-esa-copernicus-data.ipynb`
...
Apache 2.0 — See the license header in `notebook_template.ipynb`
